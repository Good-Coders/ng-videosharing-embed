	...
	.factory('RegisteredPlayers', function (PlayerConfig) {
		...
		var configurations = {
			youtube: {
				...
				protocol: 'http://',
				playerRegExp: /(http:|https:)?\/\/(?:www\.)?youtu(?:be\.com\/(?:watch\?v=|embed\/)|\.be\/)([\-\w]*)(&(amp;)?[\w\?=]*)?/
			},
			youtubeNoCookie: {
				options: {
					autoplay: 0,
			...
			vimeo: {
				...
				protocol: 'http://',
				playerRegExp: /(http:|https:)?\/\/(?:player\.)?vimeo\.com\/(?:video\/)?([A-Za-z0-9]+)/
			},
			dailymotion: {
				options: {
					autoPlay: 0,
				...
				protocol: 'http://',
				playerRegExp: /(http:|https:)?\/\/www\.dailymotion\.com\/(?:embed\/)?video\/([A-Za-z0-9]+)/
			}
		};
		var players = [];
